...
router.post('/', async (req, res) => {
    try {
        ...
        const joke = await generateShayari(keyword, genAI);
        res.json({ joke });
    } catch (error) {
 ...
 async function generateShayari(keyword, genAI) {
  try {
      ...
      const prompt = `Tell me a nice joke on ${keyword} in less than 50 words .`;
      ...
      const joke = response.text() || 'Could not generate joke at this time.';
// console.log(shayari)
      return joke;
  } catch (error) {
    if ( error.message.includes('SAFETY')) {
      console.log('Joke generation failed due to safety concerns,try with a different keyword:', error);
      return 'Joke generation failed due to safety concern. Please try a different keyword.';
    }else{
