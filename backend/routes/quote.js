...
router.post('/', async (req, res) => {
    try {
        ...
        const quote = await generateShayari(keyword, genAI);
        res.json({ quote });
    } catch (error) {
 ...
 async function generateShayari(keyword, genAI) {
  try {
      ...
      const quote = response.text() || 'Could not generate quote at this time.';
// console.log(shayari)
      return quote;
  } catch (error) {
    if ( error.message.includes('SAFETY')) {
      console.log('quote generation failed due to safety concerns,try with a different keyword:', error);
      return 'quote generation failed due to safety concern. Please try a different keyword.';
    }else{
